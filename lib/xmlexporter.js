...
export function generateXML({ wikidataItem, crossrefItem, accumulatedData }) {
  ...
  if (String(openacess_info).includes("yes")) {
    ...
    abstract_info.push(""); // ##### Note: Abstract not provided because of rights missing or not listed in creative-commons (CC).
  ...
  // ##### Note: GOOD-char is keep it, whereas BAD-char is removed.
  // https://stackoverflow.com/questions/2670037/how-to-remove-invalid-utf-8-characters-from-a-javascript-string

  // Note: This is a temporary, but reasonable solution. 
  // Reason: This removes all non-ASCII characters, not invalid UTF-8. 
  // Example: "ф" is avalid UTF-8 string, but this code snippet returns "". 

  function remove_bad_char(input) {
    ...
    for (var i=0; i<input.length; i++) {
        ...
        if (input.charCodeAt(i) <= 127) {
            ...
            output += input.charAt(i); // ##### Note: append GOOD-char

        } else {

          // ##### Note: do not append BAD-char
  ...
  const elements_string_fixed = remove_bad_char(elements_string); // ##### Note: return string without BAD-char
