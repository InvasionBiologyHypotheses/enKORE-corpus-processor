...
export function generateXML({ wikidataItem, crossrefItem, accumulatedData }) {
  ...
  if (String(openacess_info).includes("yes")) {
    ...
    if (abstract_string.includes("null")) {
      ...
      abstract_info.push(String(accumulatedData.abstract));

    }

  } else {

    abstract_info.push("Note: Abstract not provided due to missing information about rights, or not listed in any creativecommons (CC).");
