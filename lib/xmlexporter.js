...
export function generateXML({ wikidataItem, crossrefItem, accumulatedData }) {
  ...
  // ######################################################
  // ##### Extract information about coverage (START) #####
  // ######################################################
  const coverage_info = [""]; // ##### Note: to be further replaced

  // ####################################################
  // ##### Extract information about coverage (END) #####
  // ####################################################
  ...
  // #################################################################################
  // ##### Passing information from the THREE main sources into elements (START) #####
  // #################################################################################
  const elements = [
    ...
    // { identifier: "SECTION", data: " # DOI # " },
    {
      ...
      data: [
        `https://scholia.toolforge.org/work/${wikidataItem["id"]}`,
  ...
  // ###############################################################################
  // ##### Passing information from the THREE main sources into elements (END) #####
  // ###############################################################################
  ...
  function remove_bad_char1(input) {
    ...
    for (var i=0; i<input.length; i++) {
        ...
        if (input.charCodeAt(i) <= 127) {
          ...
          // ##### Note: For simplicity, we will checking only the SAMPLE OF THE CHARACTER, 
          // ##### Note: and we will not check the following:
          // ##### (A) the Unicode hexadecimal number of the character
          // ##### (B) the Unicode name of the character
          // ##### (C) the entity name (as it should be entered in an HTML document)
          // ##### (D) the equivalent numeric character reference. 
          ...
          //const valid_UTC8 = 'ф';
          const char_p0 = ' ’ ‘ ′ – γ ∼ α ı С б о ‐ −   а Е е Т і г × у '; 
          const char_p1 = ' “ ” ≤ ≥ « » ¹ ³ … ∞ ° ‰ ⁻ — → ∙ ≈ ≠ ≃  ẞ β ℃ Δ ∆ ð δ ф Ф Φ ψ € Є £ λ μ ';
          const char_p2 = ' ♀ ♂ ♀ ™ ® © Ⅳ § ¥ ¡ ¤ ';  
          const char_p3 = ' ß в ь ć Č č Ç ç ¢ Đ Ð ǧ ğ н ī κ ļ ľ ń ņ Ñ ñ ň Þ þ ř л Ś ś Ș ș Ş ş Š š ł Ł ț Ý ý Ÿ ÿ ż Ž ž Ż ';
          const char_A = ' Â â Á á À à Ä ä Ã ã Å å Æ æ ą ā ă '; 
          const char_E = ' Ê ê É é È è Ë ë Ė ė Ę ę ē ě '; 
          const char_I = ' Î î Í í Ì ì Ï ï İ '; 
          const char_O = ' Ô ô Ó ó Ò ò Ö ö Õ õ Ø ø ő '; 
          const char_U = ' Û û Ú ú Ù ù Ü ü Ū ū ů ';  
    
          const valid_UTC8 = char_p0 + char_p1 + char_p2 + char_p3 + char_A + char_E + char_I + char_O + char_U;
          ...
          if (valid_UTC8.includes(input.charAt(i)) || XML_scape.includes(input.charAt(i)) || common_char.includes(input.charAt(i))) {
            ...
            output += "_"; // ##### Note: You can replace invalid char with some special marker

            // ##### Note: appending deleted character to list
            if (list_char_deleted.includes()) { 

              // Do not append character, because list already includes it;

            } else {

              list_char_deleted += input.charAt(i);
    ...
    // console.log(filename_this + `list of deleted char: ${list_char_deleted}`);

    if (list_char_deleted != "") {

      function_log_append('./logs/','Log_invalid_characters.txt',filename_this + `XML-file: ${`${wikidataItem_id_string.replaceAll("\"", "")}.xml`}`);
      function_log_append('./logs/','Log_invalid_characters.txt',filename_this + `list of deleted char: ${list_char_deleted}`);
  ...
  // ##### Note: to use some XML validator, but pontentially using in process.js because the file already exists
  // ##### https://stackoverflow.com/questions/64063935/node-js-xml-validation
